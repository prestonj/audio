...
*/
CSinglePoleFilter::~CSinglePoleFilter(void)
{
...
*/
bool __stdcall CSinglePoleFilter::initialize()
{
	// Add your code here
...
*/
bool __stdcall CSinglePoleFilter::prepareForPlay()
{
	// Add your code here:
	m_f_Yz1[0] = m_f_Yz1[1] = 0;
...
bool __stdcall CSinglePoleFilter::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	else if (m_eFilterType == HPF)
	{
		...
		// b1 = y - sqrt(y*y-1)
		// a0 = 1 - b1
		float theta = 2.0f * M_PI * m_fCornerFreq / m_nSampleRate;
		float y = 2 + cosf(theta);
		float b1 = y - sqrt(y * y - 1);
		float a0 = 1 - b1;

		pOutputBuffer[0] = a0 * pInputBuffer[0] - (b1 * m_f_Yz1[0]);
		m_f_Yz1[0] = pOutputBuffer[0];
		...
		if (uNumInputChannels == 2 && uNumOutputChannels == 2)
		{
			pOutputBuffer[1] = a0 * pInputBuffer[1] - (b1 * m_f_Yz1[1]);
			m_f_Yz1[1] = pOutputBuffer[1];
		}
	}
...
//
bool __stdcall CSinglePoleFilter::userInterfaceChange(int nControlIndex)
{
...
*/
bool __stdcall CSinglePoleFilter::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD, float fACMix, float fBDMix)
{
	// add your code here
...
*/
bool __stdcall CSinglePoleFilter::processRackAFXAudioBuffer(float* pInputBuffer, float* pOutputBuffer,
...
*/
bool __stdcall CSinglePoleFilter::processVSTAudioBuffer(float** inBuffer, float** outBuffer, UINT uNumChannels, int inFramesToProcess)
{
...
bool __stdcall CSinglePoleFilter::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity)
{
	return true;
}

bool __stdcall CSinglePoleFilter::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff)
{
	return true;
}

// uModValue = 0->127
bool __stdcall CSinglePoleFilter::midiModWheel(UINT uChannel, UINT uModValue)
{
	return true;
}
...
bool __stdcall CSinglePoleFilter::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue)
{
	return true;
}
...
*/
bool __stdcall CSinglePoleFilter::midiClock()
{
...
bool __stdcall CSinglePoleFilter::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
bool __stdcall CSinglePoleFilter::initUI()
{
	// ADDED BY RACKAFX -- DO NOT EDIT THIS CODE!!! ------------------------------ //
...
// **--0xDEA7--**
	...
	ui1->m_pUserCookedUINTData = &m_eFilterType;
	ui1->cControlUnits = "Units                                                           ";
	ui1->cVariableName = "m_eFilterType";
	ui1->cEnumeratedList = "LPF,HPF";
